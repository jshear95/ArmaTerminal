...
if(!(_cache select 0))then{
	switch true do {
		...
		case(str(_cmd)==str("RM")):{
			...
			switch(true)do{
				...
				case(str([_curDir,_rmFile] call File_fnc_getFile) == str(0)):{
	...
	switch(true)do{
		case(str(_cache select 1) == str("RM")):{			
			if(str(_cache select 4) == str(["Y"]))then{
				...
				_contents = _curDir select 1;
				_contents set[_index, ""];
				_contents = _contents - [""];
				_curDir set[1, _contents];
				
				_output = "File deleted";
			};
			
			if(str(_cache select 4) == str(["N"]))then{
